...
在 [Releases](https://github.com/CS-LX/SuperCalc/releases/latest) 下载 `SuperCalc-win-x64.zip`，完整解压后运行 `SuperCalc.App.exe`。不要只把 exe 单独拖出来。

- Windows 11 / Windows 10 2004（19041）及以上，x64。实际验证环境为 Windows 11 x64。
- 自包含发布，随包携带 .NET 与 Windows App SDK，不需要另外安装 .NET。
- 当前版本没有代码签名或安装器。
- 首次启动有三页向导；可以跳过，也可以在“体验设置”里重放。
- 右上角 **让我算数** 一键关闭推荐、求值等待与退出挽留。小窗口自动收起侧栏，导航可从 `☰` 或 `···` 打开。
...
| 首页 | Fluent 卡片、渐变横幅、PREVIEW 徽章、会员入口、云盘入口、体验健康度 |
| 输入 | 推荐数字、个性化商业灵感、占用侧栏的 CalcPilot、三种答案语气、零积分深度思考 |
| 求值 | 准备工作空间、应用圆角、兼容性检查、结果推荐、每三次计算邀请反馈 |
| 复制 | 分享优先、“显示更多选项”、传统剪贴板属性、最终才复制 |
| 历史 | 计算时间线、本地搜索之前的推荐、再次体验按钮 |
| 服务 | SuperCalc 365 虚构权益、OneNumber 假云端冲突、保留两个完全相同的 2 |
| 设置 | 多级面包屑、“显示推荐而不是广告”、相关设置、复古数字控制面板、无用兼容开关 |
| 更新 | KB000042、圆角增加 0.5 px、99% 等待、安装后实际版本不变 |
| 退出 | 生产力旅程挽留、再体验一下、星级反馈、始终可用的直接退出 |

这些是可点击的交互，不只是画在界面上的文案。假账号不要求邮箱或密码，假订阅不收费，假云端不联网，CalcPilot 是明确标注的本地剧本。
...
[调研原始出处与设计映射](docs/design-research.md) · [MIT License](LICENSE)
